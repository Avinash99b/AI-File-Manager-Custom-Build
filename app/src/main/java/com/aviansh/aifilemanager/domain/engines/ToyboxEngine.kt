...
class ToyBoxExecutionResult(
    ...
    val stderr: String? = stderr
        ?.lineSequence()
        ?.filterNot { line -> IGNORED_STDERR_PATTERNS.any { it.matches(line.trim()) } }
        ?.joinToString("\n")
        ?.takeIf { it.isNotBlank() }

    val isSuccess: Boolean get() = exitCode == 0
    val hasErrors: Boolean get() = !stderr.isNullOrBlank()

    fun getOutput(): String = buildString {
...
object ToyboxEngine {
    ...
    fun execute(command: String?): ToyBoxExecutionResult {
        val process = ProcessBuilder("/system/bin/toybox", "sh", "-c", command).start()
